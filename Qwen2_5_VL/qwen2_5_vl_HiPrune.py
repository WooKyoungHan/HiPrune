...
OBJECT_LAYER = int(os.environ['HIPRUNE_OBJECT_LAYER'])
ALPHA = float(os.environ['HIPRUNE_ALPHA'])
RETAIN = float(os.environ['HIPRUNE_QWEN_RETENTION'])
...
class Qwen2_5_VLVisionBlock(nn.Module):
    def __init__(self, config, attn_implementation: str = "sdpa") -> None:
        ...
        assert attn_implementation != "sdpa", "HiPrune do not support sdpa attention, change attn_implementation to eager or flash_attention_2"
    ...
    def forward(
        ...
        if inputs_embeds is None:
            ...
            if pixel_values is not None:
                ...
                image_embeds, attn_list = self.visual(pixel_values, grid_thw=image_grid_thw)
        ...
        if inputs_embeds.shape[1] != 1: # KV cache
            ...
            shallow_token_indices = torch.topk(shallow_attention, k=shallow_token_num).indices
            width = int(image_grid_thw[0][2] / self.config.vision_config.spatial_merge_size)
            shallow_token_indices = torch.cat([shallow_token_indices,
                                            shallow_token_indices - 1,
                                            shallow_token_indices + 1,
                                            shallow_token_indices - width,
                                            shallow_token_indices + width])
            shallow_token_indices = shallow_token_indices.clamp(0, n_image_tokens - 1)
            shallow_token_indices = torch.unique(shallow_token_indices, sorted=False)
            
            deep_token_num = visual_token_num - shallow_token_indices.shape[0]
            available_index_mask = torch.zeros(n_image_features, dtype=torch.bool, device=input_ids.device)
            available_index_mask.scatter_(0, shallow_token_indices, 1)
            
            deep_attention -= available_index_mask.int()
            
            deep_token_indices = torch.topk(deep_attention, k=deep_token_num).indices
            retain = torch.cat([shallow_token_indices, deep_token_indices])
